...
def load_datasets(dataset, data_loc):
    if dataset == 'MNIST':
        ...
        data = pd.read_csv(data_loc+'/Adult/adult45k.csv')
        ...
        X = np.load(data_loc+'/Gisette/gisette_processed_x.npy').astype(float)
        y = np.load(data_loc+'/Gisette/gisette_processed_y.npy')
        y = np.where(y==-1, 0, y)
        ...
        X = np.load(data_loc+'/ENRON/enron_count_x.npy').astype(float)
        y = np.load(data_loc+'/ENRON/enron_count_y.npy')
